

from langchain_google_genai import ChatGoogleGenerativeAI
from langchain.tools import tool


@tool
def update_profile(field: str, value: str, phone_number: str) -> str:
    """
    Updates the user's profile in the database.
    Args:
        field: The exact profile field to update (MUST be one of: 'displayName', 'title', 'bio', 'location', 'portfolioUrl', 'github', 'linkedin').
        value: The new value to set for the field.
        phone_number: The user's phone number to identify them.
    """
    try:
        # Lazy import: only import firestore if Firebase is actually initialized
        from firebase_admin import firestore, _apps
        if not _apps:
            raise RuntimeError("Firebase not initialized")

        db = firestore.client()
        # Query by phone number field so every user gets their own document
        users_ref = (
            db.collection("users")
            .where("phone_number", "==", phone_number)
            .limit(1)
        )
        docs = list(users_ref.stream())

        if docs:
            user_id = docs[0].id
        else:
            # No existing document — create one keyed on the phone number
            user_id = f"whatsapp_{phone_number.replace('+', '').replace(' ', '')}"

        db.collection("users").document(user_id).set({field: value}, merge=True)
        return f"Successfully updated {field} to '{value}'."
    except Exception as e:
        # Graceful fallback for local development without Firebase
        return (
            f"Local Test Mode: Successfully updated {field} to '{value}' "
            f"for {phone_number} (Database bypassed: {e})."
        )


@tool
def post_to_community(content: str, phone_number: str) -> str:
    """
    Creates a new post in the community forum.
    Args:
        content: The text content of the post.
        phone_number: The user's phone number to identify the author.
    """
    try:
        # Lazy import: only import firestore if Firebase is actually initialized
        from firebase_admin import firestore, _apps
        if not _apps:
            raise RuntimeError("Firebase not initialized")

        db = firestore.client()
        # Query by phone number field
        users_ref = (
            db.collection("users")
            .where("phone_number", "==", phone_number)
            .limit(1)
        )
        docs = list(users_ref.stream())

        user_id = f"whatsapp_{phone_number.replace('+', '').replace(' ', '')}"
        author_name = "WhatsApp User"

        if docs:
            user_id = docs[0].id
            author_name = docs[0].to_dict().get("displayName", author_name)

        db.collection("posts").add({
            "author_id": user_id,
            "author_name": author_name,
            "content": content,
            "created_at": firestore.SERVER_TIMESTAMP,
        })
        return "Successfully posted your message to the community."
    except Exception as e:
        # Graceful fallback for local development without Firebase
        return (
            f"Local Test Mode: Successfully posted your message to the community "
            f"(Database bypassed: {e})."
        )


def process_whatsapp_message(message: str, phone_number: str) -> str:
    """Processes an incoming WhatsApp message using the Gemini LangChain Agent."""

    # Bug fix: use a valid Gemini model identifier (gemini-flash-latest is invalid)
    llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash", temperature=0)
    tools = [update_profile, post_to_community]
    llm_with_tools = llm.bind_tools(tools)

    try:
        # Single round-trip: ask the LLM what to do
        prompt = (
            f"You are a helpful assistant on WhatsApp. "
            f"Always pass the phone_number '{phone_number}' to tools.\n"
            f"User: {message}"
        )
        response = llm_with_tools.invoke(prompt)

        # If the LLM decided to call a tool, execute it and return the result
        if response.tool_calls:
            tool_call = response.tool_calls[0]
            tool_name = tool_call["name"]
            tool_args = tool_call["args"]

            if tool_name == "update_profile":
                return update_profile.invoke(tool_args)
            elif tool_name == "post_to_community":
                return post_to_community.invoke(tool_args)

        # If no tool was called, just return the text response
        return str(response.content)
    except Exception as e:
        return f"Sorry, I encountered an error processing your request: {str(e)}"
